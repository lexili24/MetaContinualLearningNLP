...
def main():
    ...
    parser.add_argument("--training_tasks", default=['sst-2', 'mnli', 'qqp', 'qnli'],  nargs='+', type=str,
                        #choices = ['cola', 'mnli', 'mrpc', 'qnli', 'qqp', 'rte', 'snli', 'sst-2', 'sts-b', 'wnli']
                        help="Define meta-training tasks list.") # ['sst-2', 'mnli', 'qqp', 'qnli']

    parser.add_argument("--testing_tasks", default=['cola', 'mrpc', 'sts-b', 'rte'], nargs='+', type=str,
    ...
    if args.evaluate_whole_set == True: 
        ...
        print('meta testing tasks', args.meta_testing_size)
    ...
    for epoch in range(args.epoch):
        ...
        db = create_batch_of_tasks(ids, epoch, train, is_shuffle=False, batch_size=args.num_task_train, task_list = train.get_tasks())
        task_batch = next(db)
        print("\n-----------------Meta-Training Mode-----------------\n", flush=True)
        # print('ids', ids)
        ...
        if epoch % 5 == 0:
            ...
            for i, result in enumerate(acc):
                forgetting_result[idt[i]].append(result)
            forgetting_result['f1'].append(np.mean(acc))
